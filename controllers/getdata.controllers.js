...
export class getDataController {
    ...
    static async DELETE_COMPRA(req, res){
        try {
            const { IdUsuario } = req.body;
            await carritoModel.DELETE_PRODUCT(IdUsuario);
            res.status(200).json({ message: "Carro de compra borrado" });
        } catch (error) {
            console.error(error);
            res.status(500).json({ error: "Error al borrar el carro de compra" });
        }
    }
    ...
    static async CHANGECANT(req, res) {
        try {
            ...
            res
                .status(500)
                .json({ error: "Error al cambiar la cantidad de la carta" });
        }
    }
    ...
    static async PRICE_CARD(req, res) {
        try {
            ...
            let totalNeto = 0;
            axios
                .post(`${HOST}:${PORT}/vitrina/prices`, { cartas: list })
                .then(async (response) => {
                    console.log("Respuesta de la API de precios:", response.data);
                    totalNeto = response.data.reduce(
                        (acumulador, carta) => acumulador + carta.precio,
                        0
                    );
    ...
    static async INFO_CARDS(req, res) {
        try {
            ...
            axios
                .post(`${HOST}:${PORT}/inventario/getCardsbyID`, list)
                .then(async (response) => {
                    ...
                    const totalNeto = cantxprice.reduce(
